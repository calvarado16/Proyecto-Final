...
def _oid_or_str(v: str):
    try:
        ...
        return v  # si tu user.id es string, guarda string
...
async def create_profession(prof: Profession, request: Request):
    ...
    exists = col_professions.find_one({"description": prof.description})
    if exists:
        raise HTTPException(status_code=400, detail="La profesión ya existe")

    data = prof.model_dump()
    data.pop("id", None)
    data["created_by"] = user.get("id")  # guarda como string; si quieres ObjectId: _oid_or_str(user["id"])
...
async def update_profession(id: str, prof: Profession, request: Request):
    ...
    payload.pop("id", None)
    # nunca permitir cambiar el owner desde aquí
